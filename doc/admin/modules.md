...
* `restrict_section` : indique la section auquel le membre doit avoir accès pour pouvoir accéder au module, parmi `web, documents, users, accounting, connect, config`. Ceci est une valeur par défaut, et à moins de mettre `allow_user_restrict=false`, l'administrateur pourra modifier cette restriction.
* `restrict_level` : indique le niveau d'accès que le membre doit avoir dans la section indiquée pour pouvoir accéder au module, parmi `read, write, admin`.
