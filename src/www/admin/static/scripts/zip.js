...
g.downloadAsZip = async (name, files, token) => {
	...
	async function *downloadFiles() {
		for await (const f of files) {
			msg.innerText = 'Téléchargement — ' + f.name;
			yield {name: f.name + '.' + f.extension, input: await fetch(f.url, {method: 'POST', body: data})};
			// Wait a bit between files, to get around rate limiting on the server
			await new Promise(r => setTimeout(r, 1000));
		}
	}

	const zip = await clientDownloadZip(downloadFiles()).blob();
