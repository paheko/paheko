...
class CSV extends CSV_Custom
{
	...
	protected function loadQIF(string $path): void
	{
		$transactions = (new QIFParser)->parse(file_get_contents($path));

		if (empty($transactions)) {
			throw new UserException('Le fichier QIF ne comporte aucune ligne');
		}
	...
	protected function loadOFX(string $path): void
	{
		...
		if (!$account || empty($account->statement->transactions)) {
			throw new UserException('Le fichier OFX ne comporte aucune ligne');
		}
