...
class Fee extends Entity
{
	...
	protected function checkFormula(): ?string
	{
		try {
			...
			$db->prepareRestricted(['users' => null, 'services_subscriptions' => null, 'services' => null, 'services_fees' => null], $sql);
			return null;
		}
	...
	public function allUsersList(bool $include_hidden_categories = false): DynamicList
	{
		...
		$tables = 'services_subscriptions sub
			...
			INNER JOIN (SELECT id, MAX(date) FROM services_subscriptions GROUP BY id_user, id_fee) AS sub2 ON sub2.id = sub.id
